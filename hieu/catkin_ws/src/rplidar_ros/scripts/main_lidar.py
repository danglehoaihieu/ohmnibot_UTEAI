...
de2ra = 0.0174533
time_fps = time.time()
cf.out = cv2.VideoWriter('output_03.avi', cv2.VideoWriter_fourcc(*'XVID'), 30, (300, 340))
point_tracker = Tracker()
cf.yaw = 10
cf.yaw_dst = 0
cf.error_yaw = cf.yaw - cf.yaw_dst
class lidar_pro:
	def __init__(self):
		...
		self.carX = 100#240#60
		self.carY = 280#500#100
		self.angle_min = 90#100 # >0
		self.angle_max = -90#-100 # <0
		self.scale = 100
		self.clear_all()
	...
	def fix_roi(self):
		if self.yes_obstacle == 0:
			#print("error center", abs(self.error_center))
			if abs(self.error_center) > 150: self.bxa = self.hmin
			else:	self.bxa = self.hmax
			#self.bxa = self.hmax
	...
	def rotate_point(self, center, angle, point):
		cx = center[0]
		cy = center[1]
		s = np.sin(angle*de2ra);
		c = np.cos(angle*de2ra);
		px = point[0]
		py = point[1]
	...
	def tracking(self):
		...
		if not self.start_track and min_dis < 75 and h > h_roi:
	...
	def draw_map_cross(self,data):
		...
		error = cf.error_yaw
		angle_min = int(self.angle_min+error)
		angle_max = int(self.angle_max-error)
		for i,p in enumerate(roi[0]):
			roi[0][i] = self.rotate_point(center=(self.carX+self.trans,self.carY), angle=error, point=p)
		xmin = 0
		xmax = 0
		ymin = 0
		ymax = 0
		...
		for i in range(angle_min, angle_max, -1):
			x = 0
			y = 0
			dis = data_lidar.ranges[i]			
			if dis != float('inf'):
				if i < 180:	angle = i
				elif i > 180:	angle = i - 360
				x = int(self.carX+self.trans - scale*dis*math.sin(angle*de2ra))
				y = int(self.carY - scale*dis*math.cos(angle*de2ra))
		...
		cv2.drawContours(image, roi, -1, (255,255,255), 1)
	...
	def draw_map_over(self, data):
		...
		angle_max = int(self.angle_max-error)
		for i,p in enumerate(roi[0]):
			roi[0][i] = self.rotate_point(center=(self.carX,self.carY), angle=error, point=p)
		xmin = 0
		xmax = 0
		ymin = 0
		ymax = 0
		...
		self.image_draw[np.where(image[:]>10)] = [255,255,255]
		self.image_draw[np.where(image[:]<10)] = [0,0,0]
	def over_pro(self,data):
		self.draw_map_over(data)
		self.fix_roi()
		self.tracking()
		self.talker(self.yes_obstacle)
	def cross_pro(self,data):
		self.draw_map_cross(data)
	...
	def callback_lidar(self,data):
		...
		if self.allow_over or self.allow_cross:
			if self.allow_over:		self.over_pro(data)
			elif self.allow_cross:	self.cross_pro(data)
		else:
			self.talker(0)
		print('t', time.time()-t)
		font = cv2.FONT_HERSHEY_SIMPLEX
		line = cv2.LINE_AA
		cv2.putText(self.image_draw,str(self.distance)+" "+str(self.area)+" "+str(abs(self.error_center)),(10,220),font,0.6,(0,255,0),1,line)
